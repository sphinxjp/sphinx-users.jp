...
下記のような、 ``graphicx`` パッケージを読み込んでいる所をエディタで開き、上段の ``usepackage`` のところに ``[dvipdfm]`` を追加してください。
...
以上の準備ができたら、まずはDVIファイルを作ります。警告が大量にでますが、Enter連打でなんとかなります。Windwosでスタックがなくなったというエラーて進まなくなった時は、上記のMakefileに ``-kanji=utf8`` を設定するのを忘れないでください。
